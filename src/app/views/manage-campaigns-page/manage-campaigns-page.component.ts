...
export class ManageCampaignsPageComponent implements OnInit {
  ...
  private loadCampaigns(campaign?: Campaign) {
    ...
    this._dataService.getGlobalCampaigns()
      .then(cp => {
        this.campaigns = cp;
        if (campaign) {
          let index = this.campaigns.findIndex(c => c.code === campaign.code);
          this.currentCampaign = index >= 0 ? this.campaigns[index] : null;
        }
