...
export class SearchCaseCriteriaComponent implements OnInit {
  ...
  reset() {
    ...
    this.criteria.StatusCd = "";
    this.statusCheck.splice(0);
    this.criteria.FollowUpDueCd = null;
  }

  private isStatusChecked(code: string): boolean {
    return this.statusCheck.findIndex(s => s === code) > -1;
  }

  private checkStatus(code: string): void {
    let index = this.statusCheck.findIndex(s => s === code);
    if (index > -1) this.statusCheck.splice(index, 1);
    else this.statusCheck.push(code);

    this.criteria.StatusCd = "";
    for (let i in this.statusCheck) {
      this.criteria.StatusCd += this.statusCheck[i];
      if (parseInt(i) < this.statusCheck.length - 1) this.criteria.StatusCd += "*";
    }
  }
