...
import {
  AccountListInfo, CampaignListInfo, CasesListInfo, ProcessesListInfo,
  TasksListInfo
...
export class FullSearchComponent implements OnInit {
  ...
  @Output() onSearchTask = new EventEmitter<any>();
  @Output() onSearchProcesses = new EventEmitter<any>();
  ...
  onClickSearchTask(event){
    this.onSearchTask.emit(event);
  }

  onClickSearchProcesses(event){
    this.onSearchProcesses.emit(event);
  }
  ...
  private isAgentManager() {
    return this._globalStateService.loggedAgent.isManager;
  }

  private isBpmAdmin() {
    return this._globalStateService.loggedAgentHasRoleCode(ROLE_STANDARD_CODES.BPM_ADMIN);
  }
