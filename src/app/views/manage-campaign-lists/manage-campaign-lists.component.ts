...
export class ManageCampaignListsComponent implements OnInit, OnChanges {
  ...
  @Output() onRefreshCampaigns = new EventEmitter<Campaign>();
  ...
  private isChangingDefValues = false;
  ...
  ngOnChanges(changes) {
    if (changes.campaign) {
      ...
      this.isChangingDefValues = false;
    }
  }
  ...
  private newList(addDefValues: boolean = true) {
  ...
  private setDefaultValues() {
    this.waitForCreate = true;
    this._dataService.setCampaignsDefaultValues(this.campaign, this._attributeValues)
      .then(cod => {
        this.onRefreshCampaigns.emit(this.campaign);
        this.campaign = null;
        this.cancelNew();
        this._userFeedbackService.handleSuccess("Changes have been saved");
        this.waitForCreate = false;
      })
      .catch(e => {
        var msg = null;
        ...
        this._userFeedbackService.handleError(`Error setting the default values${msg ? ': ' + msg : ''}`);
        this.waitForCreate = false;
      });
  }
  ...
  private changeDefaultValues() {
    this.isChangingDefValues = true;
    this.isCreating = false;
    this._attributeValues = {};
  ...
  private addDefaultValues() {
    ...
    for (let att of this.campaign.attributes) {
      for (let val of att.defaultValue) {
        if (val) {
          this.addAttributeByCode(att.code, this.getStringValue(att.type, val));
        }
      }
    }
  }
