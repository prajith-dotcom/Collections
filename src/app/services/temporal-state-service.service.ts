...
export class TasksListInfo {
  _currentParams: SearchTaskInboxParams = null;
  _currentPagination: Pagination = new Pagination(0, 10);
  _currentSortOrder: SortOrder = new SortOrder(null, null);

  constructor(currentParams?: SearchTaskInboxParams, currentPagination?: Pagination, currentSortOrder?: SortOrder) {
    this.currentParams = currentParams;
    this.currentPagination = currentPagination;
    this.currentSortOrder = currentSortOrder;
  }

  get currentParams(): SearchTaskInboxParams {
    return this._currentParams;
  }

  set currentParams(value: SearchTaskInboxParams) {
    this._currentParams = value;
  }
...
export class ProcessesListInfo {
  _currentParams: SearchProcessesListParams = null;
  _currentPagination: Pagination = new Pagination(0, 10);
  _currentSortOrder: SortOrder = new SortOrder(null, null);

  constructor(currentParams?: SearchProcessesListParams, currentPagination?: Pagination, currentSortOrder?: SortOrder) {
    this.currentParams = currentParams;
    this.currentPagination = currentPagination;
    this.currentSortOrder = currentSortOrder;
  }

  get currentParams(): SearchProcessesListParams {
    return this._currentParams;
  }

  set currentParams(value: SearchProcessesListParams) {
    this._currentParams = value;
  }
...
export class TemporalStateServiceService{
   ...
   _tasksListInfo: TasksListInfo;
   _processesListInfo: ProcessesListInfo;
    ...
    _router.events.subscribe((event) => {
      ...
      if (event instanceof NavigationEnd) {
        if(!route.includes(PublicUrls.main.url) && !route.includes(PublicUrls.account.url)){
          ...
          this._tasksListInfo = null;
          this._processesListInfo = null;
        }
  ...
  set tasksListInfo(info: TasksListInfo) {
    this._tasksListInfo = info;
    this._urlToRedirect = PublicUrls.main.url;
  }

  get tasksListInfo(): TasksListInfo {
    return this._tasksListInfo;
  }

  set processesListInfo(info: ProcessesListInfo) {
    this._processesListInfo = info;
    this._urlToRedirect = PublicUrls.main.url;
  }

  get processesListInfo(): ProcessesListInfo {
    return this._processesListInfo;
  }
  ...
  get isTaskListInfo(): boolean {
    return this.tasksListInfo != null;
  }

  get isProcessesListInfo(): boolean {
    return this.processesListInfo != null;
  }

  get hasListInfo():boolean {
    return this.isCasesListInfo || this.isCampaignListInfo || this.isAccountListInfo || this.isCasesListInfoByAccount || this.isTaskListInfo || this.isProcessesListInfo;
  }
