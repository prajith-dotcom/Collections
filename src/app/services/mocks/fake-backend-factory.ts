...
import {custInqBody, custInqBodyPhLst1, custInqBodyPhLst2} from "../../../api/custInq";
...
import {
  bpm_cases, bpm_complete_instance_status,
  bpm_one_tasks,
  bpm_process_types, bpm_procinst,
  bpm_signals, bpm_task_coin_detail, bpm_task_image, bpm_task_variables, bpm_tasks_by_group, bpm_tasks_by_owner
} from "../../../api/bpm";
import {
  t_223_node_instances, t_223_variables, t_223_xvIsAccepted, t_223_xvIsApproved,
  t_223_xvIsValid
} from "../../../api/bpm_var_test";
import {accInqForDeposit} from "../../../api/acctInqDeposit";
import {body_acctsrch_overdrawn} from "../../../api/acctsrch_daysoverdrawn";
import {disasterSummary} from "../fema-comms.service";
import {fema_disasters} from "../../../api/fema";
...
export class FakeBackendFactory implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    return of(null).pipe(mergeMap(() => {
      ...
      // Users
      else if (request.url.indexOf('/users/v1') >= 0 && request.method === "GET") {
        console.log("FAK usercampaigns");
        return of(new HttpResponse({
          status: 200,
      ...
      // ACCTINQ
        ...
        if (request.urlWithParams.includes('accountType=D') || request.urlWithParams.includes('accountType=S')) {
          console.log("FAK ACCTINQ D");
          return of(new HttpResponse({
            status: 200,
            // body: accInqDepBody
            body: accInqForDeposit
          }));
        } else {
      ...
      // CUSTINQ
        ...
        switch (request.params.get("customerId")) {
          case "BBD9999":
            return of(new HttpResponse({
              status: 200,
              body: custInqBodyPhLst1
            }));
            break;
          case "BBD9992":
            return of(new HttpResponse({
              status: 200,
              body: custInqBodyPhLst2
            }));
            break;
          default:
            return of(new HttpResponse({
...
*/
      ...
      // KIE: instance state
      else if (request.url.indexOf('processes/instances') >= 0 && request.urlWithParams.indexOf('withVars') >= 0 && request.method === "GET") {
        console.log("KIE get instance state");
        return of(new HttpResponse({
          status: 200,
          body: bpm_complete_instance_status
        }));
      }

      // KIE: complete task
      else if (request.url.indexOf('/states/completed') >= 0 && request.method === "PUT") {
        console.log("KIE complete task");
        return of(new HttpResponse({
          status: 201
        }));
      }
      ...
      // KIE: signals
      else if (request.url.indexOf('/signals') >= 0 && request.method === "GET") {
        console.log("KIE get signals");
        return of(new HttpResponse({
          status: 200,
          body: bpm_signals
        }));
      ...
      // KIE: signal instance
      else if (request.url.indexOf('/signal/') >= 0 && request.method === "POST") {
        console.log("KIE signal instance");
        return of(new HttpResponse({
          status: 201
        }));
      }
      ...
      // KIE: complete tasks
      else if (request.url.indexOf('nodes/instances') >= 0 && request.method === "GET") {
        console.log("KIE completed tasks");
        if (request.urlWithParams.indexOf('page=0') >= 0) {
          return of(new HttpResponse({
            status: 200,
            body: t_223_node_instances
          }));
        }
        else {
          return of(new HttpResponse({
      ...
      else if (request.url.indexOf('tasks/instances') >= 0 && request.url.indexOf('owners') < 0 && request.method === "GET") {
        console.log("KIE complete tasks");
        return of(new HttpResponse({
          status: 200,
          body: bpm_one_tasks
        }));
      ...
      else if (request.url.indexOf('pam/account_processes') >= 0 && request.url.indexOf('details') < 0 && request.method === "GET") {
        console.log("bpm cases");
        return of(new HttpResponse({
          status: 200,
          body: bpm_cases
        }));
      ...
      else if (request.url.indexOf('pam/process_types') >= 0 && request.method === "GET") {
        console.log("bpm process types");
        return of(new HttpResponse({
          status: 200,
          body: bpm_process_types
        }));
      }

      else if (request.url.indexOf('pam/account_processes') >= 0 && request.method === "POST") {
        console.log("bpm cases");
        return of(new HttpResponse({
          status: 200
        }));
      ...
      // KIE: create case
      else if (request.url.indexOf('processes/SPOC.SPOC/instances') >= 0 && request.method === "POST") {
        console.log("create bpm cases");
        return of(new HttpResponse({
          status: 200,
          body: 10079
        }));
      ...
      else if (request.url.indexOf('processes/definitions') >= 0 && request.url.indexOf('tasks/users') >= 0 && request.url.indexOf('inputs') >= 0 && request.method === "GET") {
        console.log("bpm task variables");
        return of(new HttpResponse({
          status: 200,
          body: bpm_task_variables
        }));
      }

      else if (request.url.indexOf('images/processes/instances') >= 0 && request.method === "GET") {
        console.log("Bpm diagram");
        return of(new HttpResponse({
          status: 200,
          body: bpm_task_image
        }));
      ...
      else if (request.url.indexOf('instances/owners') >= 0 && request.method === "GET") {
        console.log("Tasks with owner");
        return of(new HttpResponse({
          status: 200,
          body: bpm_tasks_by_owner
        }));
      }

      else if (request.url.indexOf('instances/pot-owners') >= 0 && request.method === "GET") {
        console.log("Tasks without owner");
        return of(new HttpResponse({
          status: 200,
          body: bpm_tasks_by_group
        }));
      }

      else if (request.url.indexOf('pam/account_processes_details/') >= 0 && request.method === "GET") {
        console.log("Tasks task account");
        return of(new HttpResponse({
          status: 200,
          body: bpm_task_coin_detail
        }));
      }

      else if (request.url.indexOf('processes/instances') >= 0 && request.urlWithParams.indexOf('status=1') >= 0 && request.method === "GET") {
      ...
      else if (request.url.indexOf('variables/instances/xvIsValid') >= 0 && request.method === "GET") {
        console.log("Processes list");
        return of(new HttpResponse({
          status: 200,
          body: t_223_xvIsValid
        }));
      }

      else if (request.url.indexOf('variables/instances/xvIsApproved') >= 0 && request.method === "GET") {
        console.log("Processes list");
        return of(new HttpResponse({
          status: 200,
          body: t_223_xvIsApproved
        }));
      }

      else if (request.url.indexOf('variables/instances/xvIsAccepted') >= 0 && request.method === "GET") {
        console.log("Processes list");
        return of(new HttpResponse({
          status: 200,
          body: t_223_xvIsAccepted
        }));
      }

      else if (request.url.indexOf('variables/instances') >= 0 && request.method === "GET") {
        console.log("Processes list");
        return of(new HttpResponse({
          status: 200,
          body: t_223_variables
        }));
      }

      else if (request.url.indexOf('campaigns') >= 0 && request.method == "PUT") {
        console.log("Campaig default values");
        return of(new HttpResponse({
          status: 200
        }));
      }
      ...
      // KIE: instance state
      else if (request.url.indexOf('processes/instances') >= 0 && request.method === "DELETE") {
        console.log("KIE process aborted");
        return of(new HttpResponse({
          status: 200
        }));
      }

      else if (request.url.indexOf(disasterSummary) >= 0 && request.method === "GET") {
        console.log("FEMA Summary");
        return of(new HttpResponse({
          status: 200,
          body: fema_disasters
        }));
      ...
      else if (request.url.indexOf('custMsg') >= 0 && request.method === "PUT") {
