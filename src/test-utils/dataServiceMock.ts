...
import {BPMCase, BPMNode, BPMProcessInstance, BPMProcessType, BPMTask, BPMVariable} from "../app/models/bpm-case";

export const dataServiceMock = {

  processesList:
    [
      new BPMProcessInstance(1234, '1', 'P1', null, 1),
      new BPMProcessInstance(2, '2', 'P2', null, 0),
      new BPMProcessInstance(3, '3', 'P3', null, 1)
    ],

  completeInfo: new BPMProcessInstance(111, "123", "pn", "2.1", 1, "Cont", 1234566, "desc",
    [
      new BPMNode(1, "nn1", 111, "Cont", 1234555, "1", "Event", true),
      new BPMNode(2, "nn2", 111, "Cont", 1234555, "2", "HumanTaskNode", true),
      new BPMNode(3, "nn3", 111, "Cont", 1234555, "3", "HumanTaskNode", true)
    ],
    [
      new BPMTask(1, "tn", "ts", "td", "tsatus", 1, false, 'ao', 'cb', null, null, null, 111,
      "tpd", "Cont", 123, this.account4)
    ],
    [
      new BPMVariable("a", "Boolean"),
      new BPMVariable("b", "Boolean"),
      new BPMVariable("c", "Object"),
      new BPMVariable("d", "Object")
    ],
    ["S1", "S2"], '', this.account4
  ),
  ...
  taskList:
  [
    new BPMTask(1, 'T1', '', '', '', 1, false, '', '', null, null, null, 1234),
    new BPMTask(2, 'T2'),
    new BPMTask(3, 'T3')
  ],
  ...
  processTypes:
  [
    new BPMProcessType("1", "ProcessType1"),
    new BPMProcessType("2", "ProcessType2")
  ],
  ...
  account4:
    new Account("DIRECT", "707", "A", new Customer("1", "777", "888", "555555",
      ...
      [new AccountHistoryEntry("#post date", 77, "#effect date", "#pmt dd", "#trnCodeCode", "#trnCodeDesc", "#trnType",
        "#affCode")],
      new BPMCase("1", "707", "A", "555555", 1234)
    ),
  ...
  bpm_getProcessTypes: function(): Promise<BPMProcessType[]> {
    return new Promise<BPMProcessType[]>(s=>s(this.processTypes));
  },

  getTasksRelatedAccount: function(): Promise<Account[]> {
    return new Promise<Account[]>(s=>s([this.account4]));
  },

  completeTask: function(): Promise<number> {
    return new Promise<number>(s=>s(200));
  },

  getCompleteInfoForCase: function(): Observable<BPMProcessInstance> {
    return new Observable<BPMProcessInstance>(()=> this.completeInfo);
  },

  getProcessesRelatedAccount: function(): Promise<Account[]> {
    return new Promise<Account[]>(s=>s([this.account4]));
  },

  bpm_getCases: function(): Promise<BPMCase[]> {
    return new Promise<BPMCase[]>(s=>s(this.bpmCases));
  }
